...
class SZSERefinancing(ExchangeBase):
    """深交所再融资反馈文件抓取器。"""
    ...
    def _build_session(self) -> requests.Session:
        ...
        session.headers.update({
            ...
            "Referer": f"{self.BASE_URL}/listing/disclosure/refinancing/index.html",
        })
        return session
    ...
    def fetch_projects(self, days: int = 7) -> FeedbackReport:
        ...
        print(f"📋 Fetching SZSE refinancing since {cutoff.strftime('%Y-%m-%d')}...", file=sys.stderr)
        ...
        while True:
            ...
            for proj in data.get("data", []):
                ...
                for doc in proj.get("subInfoDisclosureList", []):
                    ...
                    # Detect refi type
                    for kw in ["可转债", "可转换"]:
                        if kw in dfnm: projects_map[company]["business_type"] = "可转债"; break
                    if projects_map[company]["business_type"] == "再融资":
                        for kw in ["配股"]:
                            if kw in dfnm: projects_map[company]["business_type"] = "配股"; break
        ...
        for info in projects_map.values():
            ...
            if reply_doc or offering_doc:
                all_projects.append(ProjectFeedback(
                    company_name=info["company"], stock_code=info["code"],
                    business_type=info["business_type"],
                    reply=reply_doc, prospectus=offering_doc,
                ))

        print(f"✅ Found {len(all_projects)} SZSE refinancing projects", file=sys.stderr)
        return FeedbackReport(
            exchange=self.EXCHANGE, business_type="refinancing",
            date_range=date_range, refi_projects=all_projects,
        )
