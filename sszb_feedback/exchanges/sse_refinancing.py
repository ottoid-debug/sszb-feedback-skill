"""SSE 再融资（增发/配股/可转债）反馈文件抓取器。

SSE 目前仅披露反馈回复和募集说明书（注册稿），不直接披露原始反馈意见。
"""
import re
import json
import time
...
class SSERefinancing(ExchangeBase):
    """上交所再融资反馈文件抓取器。"""
    ...
    def _build_session(self) -> requests.Session:
        ...
        session.headers.update({
            ...
            "Referer": "https://www.sse.com.cn/disclosure/refinancing/",
        })
        return session
    ...
    def fetch_projects(self, days: int = 7) -> FeedbackReport:
        ...
        print(f"📋 Fetching SSE refinancing since {cutoff.strftime('%Y-%m-%d')}...", file=sys.stderr)
        ...
        for info in projects_map.values():
            ...
            for item_doc in info["items"]:
                ...
                if not business_type:
                    for kw in ["可转债", "可转换"]:
                        if kw in title: business_type = "可转债"; break
                    if not business_type:
                        for kw in ["配股"]:
                            if kw in title: business_type = "配股"; break
                    if not business_type:
                        for kw in ["非公开", "向特定对象"]:
                            if kw in title: business_type = "非公开发行"; break
        ...
        print(f"✅ Found {len(all_projects)} SSE refinancing projects", file=sys.stderr)
